...
const sourceText = computed(() =>
  sourceLabel.value === 'jd' ? '京东（真实数据）' : 'Mock 种子数据（演示）',
)
...
<template>
  <div class="page">
    ...
    <el-card shadow="never">
      ...
      <el-alert
        v-if="result && result.configuration?.source === 'jd' && result.configuration?.configured === false"
        title="京东数据源不可用"
        type="warning"
        :closable="false"
        show-icon
        class="tip"
        description="京东匿名搜索被风控时需提供登录 cookie（KESE_JD_COOKIE 或 providers/state/jd_cookie.txt）。"
      />
      ...
      <el-alert
        ...
        show-icon
        class="tip"
        description="京东搜索接口不返回执行标准号/材质等字段，这些字段会如实标记为「未标注」，不会编造。接入 OCR 识别包装标准号后即可补齐核验链路。"
      />
